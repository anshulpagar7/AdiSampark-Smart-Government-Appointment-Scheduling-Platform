...
interface AppointmentPayload {
  ...
  mobile?:              string;
  notes?:               string;
  meeting_link?:        string;      // online meeting URL (Teams, Meet, Zoom, Webex…)
}
...
function buildEventPayload(data: AppointmentPayload): Record<string, unknown> {
  ...
  // ── Meeting link section (appended to description if present) ──────────────
  const meetingLinkSection = data.meeting_link?.trim()
    ? [``, `------------------------------------`, `🔗 Online Meeting`, data.meeting_link.trim(), `------------------------------------`]
    : [];

  // Google Meet links also set the location so Calendar shows the native "Join" button
  ...
  // Location: Google Meet URL for native join button, otherwise physical office
